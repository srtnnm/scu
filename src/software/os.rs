...
pub fn get_name() -> String {
  ...
  if !fs::metadata( "/etc/os-release" ).is_ok() { return result; }

  for line in fs::read_to_string( "/etc/os-release" ).unwrap()
    .split( '\n' ) {
...
pub fn get_uptime() -> utils::converter::Time {
  let time: i32 = fs::read_to_string( "/proc/uptime" ).unwrap()
    .split( '.' )
    .next()
    .unwrap()
    .parse::<i32>()
...
pub fn get_shell() -> String {
  String::from( env::var( "SHELL" ).unwrap_or( String::from( "/" ) )
    .split( '/' )
    .next_back()
    .unwrap()
  )
